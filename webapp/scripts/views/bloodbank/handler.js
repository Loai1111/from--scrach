        ...
        try {
            switch (state.currentView) {
                case 'requests':
                    ...
                    break;
                case 'donations':
                    const [donations, donors, bloodBags] = await Promise.all([
                    ...
                    const bagsMap = new Map(bloodBags.map(bag => [bag.id, bag]));

                    data = donations.map(donation => {
                        const bag = bagsMap.get(donation.bloodBagId);
                        return {
                            ...donation,
                            bloodType: bag ? bag.bloodType : 'N/A'
                        };
                    });
                    break;
                case 'inventory':
                    data = await services.inventory.getAllBloodBags();
                    break;
                case 'donors':
                    data = await services.donor.getDonors();
                    break;
                case 'notifications':
                    data = await services.notification.getNotifications('bloodbank');
                    break;
            }
            console.log(`[Data Fetch] Successfully fetched ${data.length} items for view: ${state.currentView}`);
        } catch (error) {
    ...
    const filtered = sourceData.filter(item => {
        ...
        if (!matchesSearch) {
            if (isTests) {
                ...
                switch (state.currentView) {
                    case 'requests':
                        matchesSearch = item.patientId?.toLowerCase().includes(searchTerm) || item.id.toLowerCase().includes(searchTerm);
                        break;
                    case 'donations':
                        matchesSearch = item.id.toLowerCase().includes(searchTerm) || item.donorId?.toLowerCase().includes(searchTerm) || item.bloodBagId?.toLowerCase().includes(searchTerm) || item.donorName?.toLowerCase().includes(searchTerm);
                        break;
                    case 'inventory':
                        matchesSearch = item.id.toLowerCase().includes(searchTerm) || item.donorId?.toLowerCase().includes(searchTerm);
                        break;
                    case 'donors':
        ...
        if (!matchesStatus) {
            const itemStatus = item.status || item.result || (item.disqualificationStatus || 'None');
...
function handleTestTabClick(e) {
    ...
    ui.renderTestsPage(applyFilters);
}
...
function handleDetailsActionClick(e) {
    ...
    const actionHandlers = {
        'start-crossmatch-btn': handleStartCrossmatch,
        'mark-compatible-btn': handleMarkCompatible,
        'mark-incompatible-btn': handleMarkIncompatible,
...
async function handleStartCrossmatch() {
    if (!state.selectedItemId) { alert("No request selected."); return; }
    const selectedRequest = state.allPageData.find(r => r.id === state.selectedItemId);
    if (!selectedRequest) { alert("Could not find selected request."); return; }

    const allocated = selectedRequest.allocatedCount || 0;
    const pending = selectedRequest.pendingCount || 0;
    const limit = selectedRequest.quantity - (allocated + pending);
    if (limit <= 0) {
        alert("This request has already been fully allocated or is in progress.");
        return;
    }
    const patientBloodType = selectedRequest.bloodType;
    const compatibleTypes = bloodCompatibility[patientBloodType] || [];

    document.getElementById('cm-request-info').textContent = `${selectedRequest.quantity} unit(s) of ${patientBloodType} for patient ${selectedRequest.patientId}`;
    
    const allAvailableBags = await services.inventory.getAvailableBloodBags();
    let compatibleBags = allAvailableBags.filter(bag => compatibleTypes.includes(bag.bloodType));

    // Rank the compatible bags
    const patient = state.patientsCache.find(p => p.id === selectedRequest.patientId);
    if (patient) {
        compatibleBags = services.rankingService.rankCompatibleBags(patient, compatibleBags);
    }

    const cmBagList = document.getElementById('cm-bag-list');
    cmBagList.innerHTML = '';
    cmBagList.onchange = () => updateCrossmatchCounter(limit);
    ...
    if (compatibleBags.length === 0) {
        ...
        compatibleBags.forEach(bag => {
            ...
            div.className = 'flex items-center justify-between p-3 hover:bg-gray-100 rounded-lg border-b';
            div.innerHTML = `
                <div class="flex-grow">
                    <p class="font-semibold">${bag.id}</p>
                    <p class="text-sm text-gray-600">Type: ${bag.bloodType} | Expires: ${bag.expiryDate ? bag.expiryDate.toDate().toLocaleDateString() : 'N/A'}</p>
                </div>
                <div class="text-center px-4">
                    <p class="font-bold text-lg text-red-700">${Math.round(bag.totalScore)}</p>
                    <p class="text-xs text-gray-500">Score</p>
                </div>
                <input type="checkbox" data-bag-id="${bag.id}" class="cm-checkbox h-5 w-5 text-red-600 border-gray-300 rounded focus:ring-red-500">
            `;
            cmBagList.appendChild(div);
        });
    }

    updateCrossmatchCounter(limit);
    document.getElementById('crossmatch-modal').classList.remove('opacity-0', 'pointer-events-none');
}
...
async function handleSubmitCrossmatch() {
    const cmSubmitBtn = document.getElementById('cm-submit-btn');
    const selectedBags = Array.from(document.querySelectorAll('#cm-bag-list .cm-checkbox:checked')).map(cb => cb.dataset.bagId);
    if (selectedBags.length === 0) { alert("Please select at least one blood bag."); return; }
    try {
        cmSubmitBtn.disabled = true;
        cmSubmitBtn.textContent = 'Submitting...';
        await services.crossmatch.createCrossmatchTests(state.selectedItemId, selectedBags);
        await services.request.recalculateRequestStatus(state.selectedItemId);
        alert(`Successfully submitted ${selectedBags.length} bag(s) for crossmatching.`);
        document.getElementById('crossmatch-modal').classList.add('opacity-0', 'pointer-events-none');
        fetchAndDisplayData();
    } catch (error) { console.error("Error submitting crossmatch:", error); alert("An error occurred."); } finally {
        cmSubmitBtn.disabled = false;
        cmSubmitBtn.textContent = 'Submit Crossmatch';
    }
}
...
async function handleMarkCompatible() {
    const test = state.getSelectedTestItem();
    if (!test || !test.bloodBagId) return;

    const compatibleBtn = document.getElementById('mark-compatible-btn');
    const incompatibleBtn = document.getElementById('mark-incompatible-btn');

    if (compatibleBtn) compatibleBtn.disabled = true;
    if (incompatibleBtn) incompatibleBtn.disabled = true;
    if (compatibleBtn) compatibleBtn.textContent = 'Updating...';

    try {
        const reportUrl = await handleFileUpload();
        await services.crossmatch.updateCrossmatchResult(test.id, 'Compatible', test.bloodBagId, reportUrl);
        
        alert(`Test ${test.id} marked as Compatible.`);
        ...
        if (error.message.includes('cancelled')) {
            ...
            console.error("Error during compatibility marking:", error);
            alert("An error occurred. Please try again.");
        }
    } finally {
        // This block ensures buttons are always restored.
        if (compatibleBtn) {
            compatibleBtn.disabled = false;
            compatibleBtn.textContent = 'Mark as Compatible';
        }
        if (incompatibleBtn) {
            incompatibleBtn.disabled = false;
        }
    }
}

async function handleMarkIncompatible() {
    const test = state.getSelectedTestItem();
    if (!test || !test.bloodBagId) return;
    ...
    if (compatibleBtn) compatibleBtn.disabled = true;
    if (incompatibleBtn) incompatibleBtn.disabled = true;
    if (incompatibleBtn) incompatibleBtn.textContent = 'Updating...';

    try {
        const reportUrl = await handleFileUpload();
        await services.crossmatch.updateCrossmatchResult(test.id, 'Incompatible', test.bloodBagId, reportUrl);

        alert(`Test ${test.id} marked as Incompatible.`);
        await fetchAndDisplayData();
        ui.renderTestsPage();
    } catch (error) {
        if (error.message.includes('cancelled')) {
            ...
            console.error("Error during incompatibility marking:", error);
            alert("An error occurred. Please try again.");
        }
    } finally {
        if (compatibleBtn) {
        ...
        if (incompatibleBtn) {
            incompatibleBtn.disabled = false;
            incompatibleBtn.textContent = 'Mark as Incompatible';
        }
    }
}

// The old upload and close functions are no longer needed as standalone handlers.
// They are replaced by the new promise-based handleFileUpload function.
...
async function handleScreeningSubmit(e) {
    ...
    const [systolic, diastolic] = document.getElementById('s-bp').value.split('/').map(s => parseInt(s.trim()));
    ...
    const result = issues.length === 0 ? 'Pass' : 'Temporary Disqualification';
    ...
    try {
        await services.healthScreening.addHealthScreening({
            ...
            skinInspectionNotes: document.getElementById('s-skin-notes').value,
            result,
        });
...
function initializeEventListeners() {
    ...
    // Modals
    document.getElementById('cm-cancel-btn').addEventListener('click', () => document.getElementById('crossmatch-modal').classList.add('opacity-0', 'pointer-events-none'));
    ...
    document.getElementById('cm-submit-btn').addEventListener('click', handleSubmitCrossmatch);
    ...
    document.getElementById('tests-tab-content').addEventListener('click', handleTestTableClick);
